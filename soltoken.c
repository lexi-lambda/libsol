...
const sol_token DEFAULT_TOKEN = {
    {
        TYPE_SOL_TOKEN, NULL, NULL, NULL
    }, NULL
};

SolToken sol_token_create(char* identifier) {
    SolToken new_token = malloc(sizeof(*new_token));
    memcpy(new_token, &DEFAULT_TOKEN, sizeof(*new_token));
    new_token->identifier = strdup(identifier);
    return new_token;
}
